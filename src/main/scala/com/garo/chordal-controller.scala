...
case class ChordalControllerExtension(
    defn: ChordalControllerExtensionDefinition
)(implicit host: ControllerHost)
    extends ControllerExtension(defn, host)
    with ShortMidiMessageReceivedCallback {
  lazy implicit val browser = MyBrowser(host.createPopupBrowser())
  lazy val noteInput = host.getMidiInPort(0).createNoteInput("Instrument", "000000")
  lazy val transport = host.createTransport()
  lazy val arranger = host.createArranger()
  lazy val application = host.createApplication()
  lazy val track = host.createCursorTrack(0, 1)
  lazy val clipSlot = track.clipLauncherSlotBank().getItemAt(0)
  lazy val clip = track.createLauncherCursorClip(16, 16)
  ...
  lazy val blockSizeSetting = doc.getNumberSetting("Block Size", "Config", 0, 1000, 1, "ms", 24)
  def blockSize() = blockSizeSetting.getRaw().round
  ...
  override def init() = {
    ...
    clipSlot.isRecordingQueued.markInterested()

    clip.getLoopStart.markInterested()
    clip.getLoopLength.markInterested()
    clip.isPinned.set(true)
    clipSlot.isRecording.addValueObserver(new BooleanValueChangedCallback {
      override def valueChanged(isRecording: Boolean) = {
        if (!isRecording) {
          val q = transport.getClipLauncherPostRecordingTimeOffset.get()
          ...
          clip.getLoopLength.set(q)
          postFrame(() => {clip.exists.get(); clipSlot.launch()})
        }
      }
    })
...
object Helpers {
  def toBin(list: Seq[Int]) = {
    var pow = 1
    list.fold(0)((acc, i) => { val result = acc + i * pow; pow *= 2; result })
  }
}

object CursorFns {
  def selectNext(cursor: Cursor): Unit = { cursor.selectNext() }
...
class MyDevice(device: bitwig.Device)(implicit host: ControllerHost, browser: MyBrowser) {
  ...
  def initBrowsing() = {
    if (!browser.exists()) {
      if (!device.exists().get()) device.deviceChain().endOfDeviceChainInsertionPoint().browse()
      else device.replaceDeviceInsertionPoint().browse()
      browser.browsePresets()
      browser.browseInstruments()
    }
  }

  def prevPreset() = { initBrowsing(); browser.prevResult() }
  def nextPreset() = { initBrowsing(); browser.nextResult() }
  def prevCollection() = { initBrowsing(); browser.prevCollection() }
  def nextCollection() = { initBrowsing(); browser.nextCollection() }
  def commitPreset() = browser.commit()
}

case class MyBrowser(browser: PopupBrowser) {
  browser.exists().markInterested()
  browser.selectedContentTypeIndex().markInterested()
  val resultBank = browser.resultsColumn().createItemBank(1)
  val result = resultBank.getItemAt(0)
  val collectionBank = browser.smartCollectionColumn().createItemBank(1)
  val collection = collectionBank.getItemAt(0)
  val deviceTypeBank = browser.deviceTypeColumn().createItemBank(4)

  def exists() = browser.exists().get()
  def browsePresets() = browser.selectedContentTypeIndex().set(1)
  def browseInstruments() =
    (0 until deviceTypeBank.getSizeOfBank()) map ((i) => deviceTypeBank.getItemAt(i).isSelected().set(i == 2))

  def prevResult() = { resultBank.scrollBackwards(); result.isSelected.set(true) }
  def nextResult() = { resultBank.scrollForwards(); result.isSelected.set(true) }

  def prevCollection() = { collectionBank.scrollBackwards(); collection.isSelected.set(true) }
  def nextCollection() = { collectionBank.scrollForwards(); collection.isSelected.set(true) }
...
case class MyBitwigDevice(spec: DeviceSpec, device: bitwig.Device)(implicit host: ControllerHost, browser: MyBrowser)
  ...
  def insert(at: InsertionPoint) = at.insertBitwigDevice(spec.id)

  def getParameter(id: String) = paramMap(id)
}

object MyDevice {
  def findFirstBitwigDevice(track: CursorTrack, spec: DeviceSpec)(implicit
      host: ControllerHost,
      browser: MyBrowser
  ) = {
    val bank = track.createDeviceBank(1)
    bank.setDeviceMatcher(host.createBitwigDeviceMatcher(spec.id))
    MyBitwigDevice(spec, bank.getDevice(0))
  }

  def findFirstDevice(track: CursorTrack, matcher: DeviceMatcher)(implicit
      host: ControllerHost,
      browser: MyBrowser
  ) = {
    val bank = track.createDeviceBank(1)
    bank.setDeviceMatcher(matcher)
    new MyDevice(bank.getDevice(0))
  }

  def deviceAt(track: CursorTrack, index: Int)(implicit host: ControllerHost, browser: MyBrowser) = {
    val bank = track.createDeviceBank(100)
    new MyDevice(bank.getDevice(index))
  }
}
...
object DeviceSpec {
  object NoteTranspose extends DeviceSpec {
    val id = UUID.fromString("0815cd9e-3a31-4429-a268-dabd952a3b68")
    val octaves = "OCTAVES"
    val params = Seq(octaves)
  }
}
...
class ChordalControllerExtensionDefinition extends ControllerExtensionDefinition {
  ...
  override def createInstance(host: ControllerHost) = ChordalControllerExtension(this)(host)
}
