...
sealed abstract class Value:
  ...
  def /[V <: Value](field: Obj.Field[V]) = this match {
    case Value.Object(tpe, values) => values.find(_.id == field.tag).map(_.value.asInstanceOf[V])
    case _                         => Option.empty
  }
  def /![V <: Value](field: Obj.Field[V]) =
    this.asInstanceOf[Value.Object].values.find(_.id == field.tag).get.value.asInstanceOf[V]
end Value
...
def rcTargetsWithTag(tag: String, value: Value) = value.walkObjs(
  Obj.RCPage,
  {
    case obj if (obj /! Obj.RCPage.Tags).value.split(",").contains(tag) =>
      (obj /! Obj.RCPage.RCs).value.map(_ /! Obj.RemoteControl.Target).map(_.value)
  }
)
